    ...
    switch (isLike) {
      case true:
        if (localReaction.value) {
          ...
          const responseContext = props.context == `comment` ? `/comments/${commentId}/vote` : `/videos/${videoId.value}/vote`
          ...
          const responseContext = props.context == `comment` ? `/comments/${commentId}/vote` : `/videos/${videoId.value}/vote`
          const response = await api.post(responseContext, true)
        }
        break
      case false:
        if (localReaction.value === false) {
          ...
          const responseContext = props.context == `comment` ? `/comments/${commentId}/vote` : `/videos/${videoId.value}/vote`
          const response = await api.post(responseContext, false)
        }
        break
    }
  } catch (error) {
...
<template>
  <div class="control-button">
    ...
    <button :disabled="isProcessing" @click="handleReaction(false)" class="reaction-btn dislike control-button">
      <svg xmlns="http://www.w3.org/2000/svg" width="10" height="30" viewBox="-1 -1 12 32">
        <path :class="{'active': localReaction === false}" d="M10 0 L10 30 L5 30 L5 15 L0 15 L10 0 Z"
