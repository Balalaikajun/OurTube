<script setup>
    ...
    const uploadVideoModal = ref(null);

    const openAccount = () => {
        accountModal.value.openMenu();
    }
    const openUpload = () => {
        uploadVideoModal.value.openMenu();
    }
...
<template>
    <MasterHead v-if="showHeader" @open-upload="openUpload" @open-account="openAccount"/>
    <AccountModal ref="accountModal" v-if="showHeader"/>
    <UploadVideoModal ref="uploadVideoModal" v-if="showHeader"/>
