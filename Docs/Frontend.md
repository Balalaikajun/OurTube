...
9. Реализовать ограниченное по числу попыток автоматическое повторение запросов при сбое или неудачных запросах. Если ответ так и не получен, пользователю предоставляется кнопка «Повторить» для ручного повторного запроса. При потере соединения, создавать очередь запросов для отправки после восстановления соединения.